import React from "react";

export default function Footer() {
  return (
    <>
      {/* <!-- Remove the container if you want to extend the Footer to full width. --> */}
      <div className=" my-5">
        {/* <!-- Footer --> */}
        <footer className="text-center text-lg-start text-white w-100 position-absolute bg-dark">
          {/* <!-- Grid container --> */}
          <div className="container p-4 pb-0">
            {/* <!-- Section: Links --> */}
            <section className="">
              {/* <!--Grid row--> */}
              <div className="row">
                {/* <!--Grid column--> */}

                {/* <!--Grid column--> */}
                <div className="col-lg-1 col-md-6 mb-4 mb-md-0">
                  <h5 className="text-uppercase text-muted font-size-12">
                    About
                  </h5>

                  <ul className="list-unstyled mb-0">
                    <li>
                      <a href="#!" className="text-white">
                        Contact Us
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        About Us
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Careers
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Flipkart Stories
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Press
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Flipkart
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Wholesale
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Corporate
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Information
                      </a>
                    </li>
                  </ul>
                </div>
                {/* <!--Grid column--> */}

                {/* <!--Grid column--> */}
                <div className="col-lg-1 col-md-6 mb-4 mb-md-0">
                  <h5 className="text-uppercase text-muted font-size-12">
                    Help
                  </h5>

                  <ul className="list-unstyled mb-0">
                    <li>
                      <a href="#!" className="text-white">
                        Payments
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Shipping
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Cancellation
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        FAQ
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Infringement
                      </a>
                    </li>
                  </ul>
                </div>
                {/* <!--Grid column--> */}

                {/* <!--Grid column--> */}
                <div className="col-lg-1 col-md-6 mb-4 mb-md-0">
                  <h5 className="text-uppercase text-muted font-size-12">
                    Policy
                  </h5>

                  <ul className="list-unstyled mb-0">
                    <li>
                      <a href="#!" className="text-white">
                        Return Policy
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Terms of use
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Security
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Privacy
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Sitemap
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        EPR Compliance
                      </a>
                    </li>
                  </ul>
                </div>
                {/* <!--Grid column--> */}

                {/* <!--Grid column--> */}
                <div className="col-lg-2 col-md-6 mb-4 mb-md-0">
                  <h5 className="text-uppercase text-muted font-size-12">
                    Social
                  </h5>

                  <ul className="list-unstyled mb-0">
                    <li>
                      <a href="#!" className="text-white">
                        Facebook
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Twitter
                      </a>
                    </li>
                    <li>
                      <a href="#!" className="text-white">
                        Youtube
                      </a>
                    </li>
                  </ul>
                </div>

                <div className="col-lg-3 col-md-4 mb-4 mb-md-0 footer-mail">
                  <p className=" text-muted footer-text ">Mail Us:</p>

                  <div>
                    <p className="mb-0">Flipkart Internet Private Limited, </p>
                    <p className="mb-0"> Buildings Alyssa, Begonia &amp; </p>
                    <p className="mb-0"> Clove Embassy Tech Village, </p>
                    <p className="mb-0">
                      Outer Ring Road, Devarabeesanahalli Village,{" "}
                    </p>
                    <p className="mb-0"> Bengaluru, 560103, </p>
                    <p className="mb-0"> Karnataka, India</p>
                  </div>
                </div>
                <div className="col-lg-3 col-md-6 mb-4 mb-md-0">
                  <p className=" text-muted footer-text">
                    Registered Office Address:
                  </p>

                  <div className="">
                    <p className="mb-0">Flipkart Internet Private Limited, </p>
                    <p className="mb-0"> Buildings Alyssa, Begonia &amp; </p>
                    <p className="mb-0"> Clove Embassy Tech Village, </p>
                    <p className="mb-0">
                      Outer Ring Road, Devarabeesanahalli Village,{" "}
                    </p>
                    <p className="mb-0"> Bengaluru, 560103, </p>
                    <p className="mb-0"> Karnataka, India </p>
                    <p className="mb-0"> CIN : U51109KA2012PTC066107 </p>
                    <p className="mb-0">
                      Telephone: <a href="tel:18002029898">044-45614700</a>
                    </p>
                  </div>
                </div>
                {/* <!--Grid column--> */}
              </div>
              {/* <!--Grid row--> */}
            </section>
            {/* <!-- Section: Links --> */}

            <hr className="mb-4" />
          </div>
          {/* <!-- Grid container --> */}
          <div className="d-flex align-content-center align-items-center footer-last-font mx-lg-5 justify-content-evenly align-items-center p-3">
            <div className="ms-lg-5 ">
              <img src="data:image/svg+xml;base64," />
              <a href="https://seller.flipkart.com/?utm_source=fkwebsite&amp;utm_medium=websitedirect">
                <span className="ms-1">Become a Seller</span>
              </a>
            </div>
            <div className="ms-lg-5 ms-0">
              <img src="data:image/svg+xml;base64," />
              <a href="https://brands.flipkart.com">
                <span className="ms-1">Advertise</span>
              </a>
            </div>
            <div className="ms-lg-5 ms-0">
              <img src="data:image/svg+xml;base64," />
              <a href="/the-gift-card-store?otracker=undefined_footer_footer_navlinks">
                <span className="ms-1">Gift Cards</span>
              </a>
            </div>
            <div className="ms-lg-5 ms-0">
              <img src="data:image/svg+xml;base64," />
              <a href="/helpcentre?otracker=undefined_footer_footer_navlinks">
                <span className="ms-1">Help Center</span>
              </a>
            </div>
            <span className="ms-lg-5 ms-0">
              ©2022 <span className="">Shehroz,Ekansh & Digvijay</span>
            </span>
            <img
              src="https://static-assets-web.flixcart.com/fk-p-linchpin-web/fk-cp-zion/img/payment-method_69e7ec.svg"
              className="img-fluid footer-img"
            />
          </div>
          {/* <!-- Copyright --> */}
        </footer>
        {/* <!-- Footer --> */}
      </div>
      {/* <!-- End of .container --> */}
    </>
  );
}
